...
\input{header_beamer}
\subtitle{Topic 3: Predicates and arguments part 1}
...
\frame{
    Recap from LEL1A (see also Chapter 3 of the textbook):

\bigskip
    Semantic roles: semantic features that characterise entities. For example:
    
    \begin{itemize}
        \item \textbf{Agent:} Animate entity that initiates an event; it ``does'' things.
        \item \textbf{Patient:} Animate entity that undergoes an event.
        \item \textbf{Theme:} Inanimate entity that undergoes an event.
        \item \textbf{Goal:} Entity that receives something in an event.
        \item \textbf{Predicate:} Usually a verb or adjective. The label of the event, which might require some semantic roles (entities).
        \item \textbf{Argument:} An obligatory entity or phrase that a predicate requires.
    \end{itemize}
}

% Verbs require a certain number of (syntactic) arguments. Verbs that require one argument are intransitive (or 1-place) verbs (e.g. fall). Verbs that require two arguments, such as like, are transitive (or 2-place) verbs. Many languages also have ditrans- itive (or 3-place) verbs, which require three arguments (e.g. give).

% Arguments are the obligatory entities or phrases a verb requires

% Semantic roles are bundles of semantic features which characterise entities. An agent is animate and initiates an event. A patient (or theme) can be animate or inanimate and is the undergoer of an event. An agent ‘does’ things and things ‘are done’ to a patient. A recipient is an animate entity which receives something. See Kroeger (2005: 55–56) for further roles!
...
\bigskip
    \textbf{It} looks like every English clause needs to have a (syntactic) subject, even if it doesn't have a (semantic) agent.
}
...
\bigskip
    You have a total of three attempts to show that you've acquired any of the Syntax Skills: two in \alert{Learn exercises} and one in the tutorial submission.
