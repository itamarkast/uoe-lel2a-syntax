...
\begin{itemize}
    ...
    \item If you're still ``missing'' a Syntax Skill from a previous week and believe your tree demonstrates it, include a very brief note (1--2 sentences) after your tree mentioning which skill it was.
...
The sentences!
\begin{exe}
    \ex[]{A friend's car seemed to be stolen last night. \label{ex:4-car} \hfill \ref{np_dp} $\Box$, \ref{passives} $\Box$, \ref{raising_control} $\Box$\\{[}This is grammatical in some varieties of English, but not in mine! Don't worry too much about \emph{to be}: use two \obar{I} heads, or just treat \emph{be stolen} as one \obar{V}. You can also treat \emph{last night} as one AdvP.]
    % Whose bag did you try to steal? \label{whose_bag} \hfill \ref{np_dp} $\Box$, \ref{raising_control} $\Box$, \ref{passives} $\Box$, \ref{wh_movement} $\Box$\\
    }
    \ex[]{Dr Ali convinced her friend to take his bag with the instruments. \label{ex:4-bag} \hfill \ref{np_dp} $\Box$, \ref{passives} $\Box$, \ref{raising_control} $\Box$, \ref{A_movement} $\Box$
    % Dr Ali seems to forget which bag her instruments were put in.\label{which_bag} \hfill \ref{raising_control} $\Box$, \ref{A_movement} $\Box$, \ref{passives} $\Box$, \ref{wh_movement} $\Box$\\
    }
    ...
    {[}If you want a version without \emph{wh}-movement (optional skill 11a), that's fine too - but you'll need to give the appropriate minimally-different declarative sentence!]
        }
\end{exe}
...
{
\begin{exe}
\exi{(1)}{ % A friend's car was attempted to be stolen last night.
    \begin{forest}
    [CP
        [\phantom{a} ]
        [\ibar{C}
            [\obar{C} ]
            [IP
                [DP [a friend's car, roof, name=matrixIP]]
                [\ibar{I}
                    [\obar{I} ]
                    [VP
                        [\phantom{agent} ]
                        [\ibar{V}
                            [\obar{V}\\seemed ] 
                            [IP
                                [DP [\sout{a friend's car}, roof, name=embedIP]]
                                [\ibar{I}
                                    [\obar{I}\\to ]
                                    [VP
                                        [\phantom{agent} ]
    ...
    \draw[->,dotted] (passivetrace) to[out=south west,in=south] (embedIP);
    \draw[->,dotted] (embedIP) to[out=south west,in=south] (matrixIP);
    \end{forest}
}
\end{exe}
This is a raising construction which embeds a passive. The Theme raises (moves) twice: once for the passive movement and once for raising.
}
\end{answer}
...
{
% \begin{exe}
% \exi{(2)}{ %Dr Ali convinced her friend to take his bag with the instruments.
(2)\\
\begin{forest}
    [IP
    ...
    \draw[->,dotted] (subjectmid) to[out=south west,in=south west] (subjectcopy);
    \draw[->,dotted] (PROtrace) to[out=south,in=south] (PROsubj);
\end{forest}\\
% }
% \end{exe}
This is an object control construction. We could also imagine another analysis of \emph{her friend} which preserves binary branching -- see the course notes. The important thing here is to recognise that \emph{her friend} controls PRO, which is the agent of taking the bag.
}
\end{answer}
...
\begin{answer}
{
(3)\\ %Which stories about the pandemic did the historian consider writing an analysis of?\\
\hspace*{-24em}
\begin{footnotesize}
\begin{forest}
    [CP
