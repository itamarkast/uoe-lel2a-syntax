...
\begin{center}
    ...
    \end{forest}
\end{center}
The tree above represents the PP \emph{on alcohol} as a complement (sister of \obar{N}), rather than an adjunct (sister of N$'$).
...
\textcolor{red}{This exercise \textbf{RELIES} on judgments in English that are clear in my variety at least but this may not be the case in all of your varieties.
...
\textcolor{red}{In my variety of English, \emph{one}-replacement in the (\ref{NCC_sub}) cases is clearly unacceptable:
\begin{exe}
    \ex{
    \begin{xlist}
...
\end{exe}}
\textcolor{red}{And there is a clear contrast with the examples with relative clauses:
\begin{exe}
    \ex{
    \begin{xlist}
...
\end{exe}}
\textcolor{red}{So this suggests that noun-complement clauses like the ones in the (\ref{NCC_sub}) examples are indeed complements of the head, while relative clauses (\ref{RC_sub}) are adjuncts.}
\end{document}